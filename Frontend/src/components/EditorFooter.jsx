...
import { Save, Download, Target, Zap, Activity, Plus } from 'lucide-react';

const EditorFooter = ({ atsScore, onSave, onDownload, data, onAddSkill, isScanning }) => {
    return (
        <div style={{
            height: '80px',
            background: '#1a202c',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '40px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: '800', color: '#fff' }}>{isScanning ? 'SCANNING...' : 'LIVE ANALYSIS'}</div>
                    </div>
                </div>
                ...
                {!isScanning && atsScore.missingKeywords && atsScore.missingKeywords.length > 0 && (
                    <>
                        <div style={{ height: '30px', width: '1px', background: '#2d3748' }}></div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxWidth: '400px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                {atsScore.missingKeywords.slice(0, 4).map(skill => (
                                    <button 
                                        key={skill}
